...
router.get('/grant', OrderCtrl.getOrdersbyGrant);
...
const upload = multer({
  ...
  router.post('/upload', upload.array('fileCollection', 5), async (req, res) => {
        ...
        const {grant, milestone,statusUI, msdFileUI, dateR, numberOfFiles, numberOfItems, orderStatus, daysSinceRequest, receivedDate, orderMadeBy, price1, price2, submission,requestDay, vendor1, vendor2, category, catalog1, catalog2, date, description,notes, msdFile} = req.body;
        var dateUII = ''; 
  ...
  router.put('/update/:id', upload.array('fileCollection', 5), async (req, res) => {
    ...
    Order.findOne({ _id: req.params.id }, (err, order) => {
        ...
        if (body.grant) order.grant = body.grant;

        if (body.grant == 'Sa2Screen'){
